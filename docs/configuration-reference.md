...
В docker-compose runtime этот файл должен быть смонтирован в контейнер как `/app/.env`,
чтобы изменения применялись обычным `docker compose restart`, без пересоздания контейнера.
При этом сами runtime-переменные должны быть явно перечислены в `docker-compose.yml`
через директиву `environment`.
