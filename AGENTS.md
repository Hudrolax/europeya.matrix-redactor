...
- текущая архитектура проекта работает только для локальных пользователей, чьи access token доступны в `Synapse DB`;
- перед redaction обязательно проверять, что пользователь всё ещё состоит в комнате;
- сообщения remote/federated отправителей не считаются поддерживаемыми этой реализацией.
